...
class ProcessImagePanel(tk.Frame):
    def __init__(self, window: tk.Tk, parent: tk.Widget, *args, **kwargs) -> None:
        ...
        # Right Sub Panel
        for F in (PreprocessPanel, TrainPanel, FourthPanel, ThirdPanel, SecondPanel, FirstPanel,):
    ...
    def __init__(
        ...
        *args, **kwargs,
        ...
        smei_button = tk.Button(
            ...
            command=lambda args='SMI': self.process(args, window, parent),
        )
        ...
        auto_all_button = tk.Button(
            ...
            width=button_width, height=button_height,
            borderwidth=0, highlightthickness=0,
            command=lambda args='ALL': self.process(args, window, parent),
        )
        ...
        preperation_button = tk.Button(
            ...
            width=button_width, height=button_height,
            borderwidth=0,  highlightthickness=0,
            command=lambda args='PREP': self.process(args, window, parent),
        )
        ...
        train_classifier_button = tk.Button(
            ...
            width=button_width, height=button_height,
            borderwidth=0,  highlightthickness=0,
            command=lambda args='TRAIN': self.process(args, window, parent),
        )
        ...
        reset_button = tk.Button(
            ...
            width=button_width, height=button_height,
            borderwidth=0, highlightthickness=0,
            command=lambda args='RESET': self.process(args, window, parent),
        )
        reset_button.grid(row=5, column=0, pady=20)
        
    def process(self, name: str, window: tk.Tk, parent: tk.Widget) -> None: 
        match name:
            case 'SMI':
                threading.Thread(target=self.SMI(window, parent)).start()
            
            case 'ALL':
                window.status = name
                threading.Thread(target=self.All(window, parent)).start()
            ...
            case 'TRAIN':
                threading.Thread(target=self.TRAIN(window, parent)).start()
            
            case 'RESET':
                threading.Thread(target=self.RESET(window)).start()
            
    def SMI(self, window: tk.Tk, parent: tk.Widget) -> None:
        if window.selected_path == None:
            window.selected_path = Path(filedialog.askdirectory())
            window.select_image.update_roots(
                name= 'select_image_treeview', 
                path= window.selected_path,
            )
            
        window.cargo = load_cargo(
            input_path= window.selected_path,
            output_path= window.selected_path.parent.absolute(),
        )
        window.select_image.update_roots(
            name= 'current_image_treeview', 
            path= window.cargo.output_path,
        )
        parent.show_frame('SecondPanel')
        
    def All(self, window: tk.Tk, parent: tk.Widget) -> None:
        input_path = filedialog.askdirectory()
        window.select_image.update_roots(
            name= 'select_image_treeview',
            path= Path(input_path),
        )
        parent.show_frame('PreprocessPanel')
        
    def TRAIN(self, window: tk.Tk, parent: tk.Widget) -> None:
        
        parent.show_frame('TrainPanel')
        
    def RESET(self, window: tk.Tk) -> None:
        window.select_image.update_roots(name= 'select_image_treeview')
        window.select_image.update_roots(name= 'current_image_treeview')
    ...
    def __init__(
        ...
        *args, **kwargs,
        ...
        HER2_image_label = tk.Label(
            master=entry_frame,
            text='HER2 MODEL', font=label_font,
        ...
        CHR17_options_label = tk.Label(
            master=entry_frame,
            text='CHR17 MODEL', font=label_font,
            bg=label_background, fg=label_foreground,
            width=25, anchor='center',
        )
        ...
        model_type_label = tk.Label(
            master=entry_frame,
            text='MODEL TYPE', font=label_font,
            bg=label_background, fg=label_foreground,
        ...
        previous_step_button = tk.Button(
            master=self,
            text='Previous Step', font=label_font,
            bg=button_background, fg=button_foreground,
            activebackground=active_background, activeforeground=active_foreground,
            borderwidth=0, highlightthickness=0,
            command= lambda: self._show_previous(parent),
        )
        ...
        next_step_button = tk.Button(
            ...
            activebackground=active_background, activeforeground=active_foreground,
            borderwidth=0,  highlightthickness=0,
            command= lambda: self._show_next(window, parent),
        )
        next_step_button.pack(side=tk.BOTTOM, fill=tk.X, pady=2, ipady=2)
    
    def _show_next(self, window: tk.Tk, parent: tk.Widget) -> None:
        window.loading_screen.show()
        threading.Thread(target=self._process(window, parent)).start()
    
    def _process(self, window: tk.Tk, parent: tk.Widget) -> None:
        process_parallel(
            cargo = window.cargo,
        ...
        if len(window.cargo.final_cell_score) > 1: 
            for cell_name, _, _ in window.cargo.final_cell_score:
                for name, id, ratio, her2, chr17, _ in window.cargo.all_cell_score:
        ...
        parent.show_frame('ThirdPanel')
        window.loading_screen.hide()
        
    def _show_previous(self, parent: tk.Widget) -> None:
        parent.show_frame('FirstPanel')
...
class ThirdPanel(tk.Frame):
    def __init__(
        ...
        *args, **kwargs,
        ...
        window.cell_her2 = ttk.Entry(master = cell_frame, width = 5)
        window.cell_her2.insert(0, '0')
        window.cell_her2.grid(row=2, column=1, pady=(8,3), sticky='w')
        ...
        window.cell_chr17 = ttk.Entry(master = cell_frame, width=5)
        window.cell_chr17.insert(0, '0')
        window.cell_chr17.grid(row=3, column=1, pady=3, sticky='w')
        ...
        window.cell_ratio = ttk.Entry(master = cell_frame, width=5)
        window.cell_ratio.insert(0, '0')
        window.cell_ratio.grid(row=4, column=1, pady=3, sticky='w')
        ...
        previous_step_button = tk.Button(
            ...
            activebackground= active_background, activeforeground= active_foreground, 
            borderwidth=0, highlightthickness=0,
            command= lambda: self._show_previous(window, parent)
        )
        previous_step_button.pack(side=tk.BOTTOM, fill=tk.X)
        next_step_button = tk.Button(
            master= self, 
    ...
    def _treeview_select(self, window: tk.Tk, event):
        ...
        if selected_item:
            ...
            window.image_display.input_image_cell(cell_id, name)
            self._update_number(window, ratio, her2, chr17)
            
        if len(window.cargo.final_cell_score) > 20:
            self.required_cell = 40
                    
        if self.previous_progress < len(window.cargo.final_cell_score):
            self.previous_progress += 1
            self._add_cell()
            
    def _update_number(self, window, ratio = 0, her2 = 0, chr17 = 0) -> None:
        window.cell_her2.delete(0, tk.END)
        window.cell_her2.insert(0, str(int(float(her2))))
        
        window.cell_chr17.delete(0, tk.END)
        window.cell_chr17.insert(0, str(int(float(chr17))))
        
        window.cell_ratio.delete(0, tk.END)
        window.cell_ratio.insert(0, str(float(ratio)))
    ...
    def _add_cell(self, window: tk.Tk):
        ...
        if len(self.final_cell_score) >= self.required_cell:
            ...
            raw_img, overlay_img = window.image_display.get_cropped_cell()
            self.final_cell_score[cell_name] = [int(window.cell_her2.get()), int(window.cell_chr17.get())]
    ...
    def _show_previous(self, window: tk.Tk, parent: tk.Widget) -> None:
...
class PreprocessPanel(tk.Frame):
    ...
    def _show_next(self, window: tk.Tk, parent: tk.Widget) -> None:
        window.loading_screen.show()
        threading.Thread(target=lambda: self._process(window, parent)).start()
    ...
    def _process(self, window: tk.Tk, parent: tk.Widget) -> None:
        ...
        for item in self.prep_treeview.get_children():
            ...
            window.loading_screen.set_status(f"Building Cargo for case: {input_path}")
            ...
            window.loading_screen.set_status(f"Segementing Signals for case: {input_path}")
            ...
            window.loading_screen.set_status(f"Calculating for case: {input_path}")
            ...
            if window.status == 'ALL':
                ...
                window.loading_screen.set_status(f"Creating report for case: {input_path}")
...
class TrainPanel(tk.Frame):
    def __init__(
        ...
        *args, **kwargs,
        ...
        # Pre-run Analysis Label
        top_label = tk.Label(master= self, text= 'Train Classifier',  bg= label_background, fg= label_foreground, font= 'Arial 12 bold', width= 25, anchor= 'center',)
        top_label.pack(side=tk.TOP, fill=tk.X, ipady=5)
    ...
    def _show_previous(self, parent) -> None:
        parent.show_frame('FirstPanel')